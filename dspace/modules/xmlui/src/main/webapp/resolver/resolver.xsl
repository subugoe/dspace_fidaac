...
<xsl:stylesheet
        ...
        <xsl:variable name="baseURL">https://thestacks.libaac.de</xsl:variable>
        ...
        <xsl:template match="resolvedLPIs">
                <resolvedLPIs>
                        <LPI>
                                ...
                                <service>FID AAC Repo</service>
                                <servicehome><xsl:value-of select="$baseURL"/></servicehome>

                <xsl:choose>
                        <!-- resolve only requests which starts with 'fidaac-' -->

                  <xsl:when test="contains($query, 'fidaac-')">
                    <xsl:variable name="lpi"><xsl:value-of select="substring-after($query, 'fidaac-')" /></xsl:variable>
